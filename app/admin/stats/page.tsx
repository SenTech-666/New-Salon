...
export default async function AdminStatsPage({
  ...
  const prevUniqueClients = new Set((prevBookings ?? []).map((b: any) => b.client_phone)).size;
  ...
  const masterStats = new Map<string, { name: string; bookingsCount: number; revenue: number }>();
  completed.forEach((b) => {
    if (!b.masters) return;
